module Analytics
  AnalyticsSnapshotRecord = ::AnalyticsSnapshotRecord
  ...
  class Infrastructure::Persistence::ActiveRecord::Repositories::AnalyticsSnapshotRepository
    def save(projection_key:, state:, kafka_offset:)
      record = AnalyticsSnapshotRecord.find_or_initialize_by(projection_key: projection_key)
      record.update!(state: state, kafka_offset: kafka_offset)
    end

    def find(projection_key)
      record = AnalyticsSnapshotRecord.find_by(projection_key: projection_key)
